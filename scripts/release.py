#!/usr/bin/env python3
"""Release plumbing for the TrustGuard Claude Code plugin.
...
PLUGIN_JSON = Path("trustguard/.claude-plugin/plugin.json")
...
ARTIFACT_PREFIX = "trustguard-claude-code"
...
def built_checksums(version: str, dist: Path = Path("dist")) -> dict[str, str]:
    ...
    for platform, suffix in PLATFORMS.items():
        artifact = f"{ARTIFACT_PREFIX}_{version}_{suffix}"

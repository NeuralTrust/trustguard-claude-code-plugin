...
// hookInput is the stdin payload Claude Code sends to every hook. Fields are a
// superset across events; unknown fields are ignored on purpose so newer
// Claude Code versions keep working.
...
// hookOutput is the stdout answer. Claude Code prefers hookSpecificOutput for
...
// toHookOutput adapts a verdict to the Claude Code event contract.
func toHookOutput(in hookInput, v verdict) hookOutput {
	...
	case "PreToolUse":
		...
		if v.permission == permissionAsk {
			msg := firstNonEmpty(v.userMessage, "A TrustGuard policy needs your approval to continue.")
			return hookOutput{
				HookSpecificOutput: &hookSpecificOutput{
					HookEventName:            "PreToolUse",
...
// mcpCallName is the JSON-RPC tools/call name. Claude Code exposes MCP tools
...
func logf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "trustguard-claude-code: "+format+"\n", args...)
}
